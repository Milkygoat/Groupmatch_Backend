...
<template>
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-cyan-50 via-white to-teal-50 px-4 py-8">
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
      <div class="absolute -top-40 -left-40 w-80 h-80 bg-teal-200 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-pulse"></div>
      <div class="absolute -bottom-40 -right-40 w-80 h-80 bg-cyan-200 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-pulse"></div>
    </div>
    ...
    <Card class="w-full max-w-md shadow-xl border-0 bg-white/80 backdrop-blur-sm relative z-10">
      <CardHeader class="space-y-1 text-center pb-2">
        ...
        <CardTitle class="text-xl font-semibold text-slate-800 pt-2">Buat Akun Baru</CardTitle>
        ...
        <form @submit.prevent="handleRegister" class="space-y-3">
          <div class="space-y-2">
            ...
            <Input
              ...
              required
              class="h-10 bg-slate-50 border-slate-200 focus:bg-white transition-colors"
            />
          </div>
